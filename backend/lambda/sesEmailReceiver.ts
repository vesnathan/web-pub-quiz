...
const ssmClient = new SSMClient({ region: process.env.SSM_REGION || 'ap-southeast-2' });
...
export const handler: SESHandler = async (event: SESEvent) => {
  ...
  for (const record of event.Records) {
    ...
    console.log(`Processing email ${messageId} from ${source} to ${recipients.join(', ')}`);

    // Only process emails from Cognito (no-reply@verificationemail.com or similar)
    if (!source.includes('cognito') && !source.includes('verification') && !source.includes('no-reply')) {
    ...
    try {
      ...
      for (const recipient of recipients) {
        ...
        if (!recipient.includes('+e2e') || !recipient.endsWith('@quiznight.live')) {

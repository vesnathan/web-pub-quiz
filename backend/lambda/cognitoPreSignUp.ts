 ...
 *
 * Case 1: External provider (Google/Facebook) sign-up when native account exists
 ...
 *   - Reject with GOOGLE_ACCOUNT_EXISTS or FACEBOOK_ACCOUNT_EXISTS error
 *   - User should sign in with the appropriate provider instead
 */
export const handler: PreSignUpTriggerHandler = async (
  ...
  if (triggerSource === "PreSignUp_SignUp") {
    try {
      ...
      // Find external provider user (Google, Facebook, etc.)
      const googleUser = existingUsers.Users?.find((user) => {
        const username = user.Username?.toLowerCase() || "";
        return username.startsWith("google_");
      });

      const facebookUser = existingUsers.Users?.find((user) => {
        const username = user.Username?.toLowerCase() || "";
        return username.startsWith("facebook_");
      });
      ...
      if (facebookUser) {
        console.log("Found existing Facebook user:", facebookUser.Username);
        throw new Error("FACEBOOK_ACCOUNT_EXISTS");
      }
    } catch (error: unknown) {
      if (
        error instanceof Error &&
        (error.message === "GOOGLE_ACCOUNT_EXISTS" ||
          error.message === "FACEBOOK_ACCOUNT_EXISTS")
      ) {
        throw error;
      }

...
/**
 * Checks if an email address is associated with a Facebook account in Cognito.
...
export const handler = async (event: LambdaPayload): Promise<boolean> => {
  ...
  try {
    ...
    // Check if any user is a Facebook account
    ...
    console.error("Error checking for Facebook account:", error);
    return false;
  }
};

...
export function SessionEndScreen({
  ...
  const animatedBadgeIds = useGameStore((state) => state.animatedBadgeIds);
  ...
  const earnedBadges: AwardBadge[] = earnedBadgeIds
    .map((id) => {
      ...
      if (!badge) {
        console.error(
          `[SessionEndScreen] Badge lookup failed for ID: "${id}" - badge not found in registry`,
        );
      }
      return badge;
    })
  ...
  if (earnedBadges.length !== earnedBadgeIds.length) {
    console.warn(
      `[SessionEndScreen] ${earnedBadgeIds.length - earnedBadges.length} badge(s) failed to load from IDs:`,
      earnedBadgeIds,
    );
  }

  // Split badges: already animated (shown during questions) vs new (awarded at set end)
  const alreadyAnimatedBadges: AwardBadge[] = [];
  const newBadges: AwardBadge[] = [];

  // Count how many of each badge ID was animated (use a copy so we can decrement)
  const animatedIdCounts = new Map<string, number>();
  for (const id of animatedBadgeIds) {
    animatedIdCounts.set(id, (animatedIdCounts.get(id) || 0) + 1);
  }

  // Split earned badges into already animated vs new
  for (const badge of earnedBadges) {
    const remainingAnimatedCount = animatedIdCounts.get(badge.id) || 0;

    if (remainingAnimatedCount > 0) {
      // This badge instance was already animated
      alreadyAnimatedBadges.push(badge);
      // Decrement so we properly handle duplicates
      animatedIdCounts.set(badge.id, remainingAnimatedCount - 1);
    } else {
      // This is a new badge (e.g., awarded at set end)
      newBadges.push(badge);
    }
  }

  console.log(`[SessionEndScreen] earnedBadgeIds:`, earnedBadgeIds);
  console.log(`[SessionEndScreen] animatedBadgeIds:`, animatedBadgeIds);
  console.log(
    `[SessionEndScreen] earnedBadges:`,
    earnedBadges.map((b) => b.name),
  );
  console.log(
    `[SessionEndScreen] alreadyAnimatedBadges:`,
    alreadyAnimatedBadges.map((b) => b.name),
  );
  console.log(
    `[SessionEndScreen] newBadges (will animate):`,
    newBadges.map((b) => b.name),
  );

  const alreadyAnimatedSkillPoints = alreadyAnimatedBadges.reduce(
  ...
  // Check if current player is a guest
  const isGuest = currentPlayerId.startsWith("guest-");
  ...
  return (
    <div className="space-y-6">
      ...
      {/* New Badges - Animated (awarded at set end, e.g., "First Set Win") */}
      {newBadges.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <Card className="bg-gradient-to-br from-purple-900/30 to-indigo-900/30 border border-purple-500/30">
            <CardBody className="p-6">
              ...
              <BadgeRevolver badges={newBadges} />
            </CardBody>
          </Card>
        </motion.div>
      )}

      {/* Already Animated Badges - Static Summary */}
      {alreadyAnimatedBadges.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: newBadges.length > 0 ? 0.6 : 0.4 }}
        >
          <BadgeUnlockSummary
            badges={alreadyAnimatedBadges}
            totalSkillPointsEarned={alreadyAnimatedSkillPoints}
          />
        </motion.div>
      )}
...
export function SessionEndSidebar({
  leaderboard,
  currentPlayerId,
}: {
  leaderboard: LeaderboardEntry[];
    ...
    <div className="space-y-4">
      ...
      >
        <Button
          ...
          className="w-full"
          onPress={() => {
            // Set leaving phase first to show loading screen during transition
            setGamePhase("leaving");
            // Small delay to let the loading screen render before resetting state
            setTimeout(() => {
              resetGame();
              router.push("/");
            }, 50);
